...
# Modified by David Hovemeyer
# I modified this script slightly so that it can successfully
# build m68k-elf cross binutils/gcc on Cygwin.  A few
# things to note:
#
# - Use the 32 bit version of Cygwin, *not* the 64 bit version
#
...
#       patch
#
# To use the script: just run it and choose "m" as the target.
#
# Once the cross binutils and gcc are built, add /usr/local/m68k-elf/bin
# to your PATH and you should be ready to go.
...
# DHH: update config.guess
(cd binutils-2.16.1 && cp /usr/share/automake-1.11/config.guess .)

echo "ROCKBOXDEV: extracting gcc-$gccver in $builddir"
tar xjf $dlwhere/gcc-core-$gccver.tar.bz2

# DHH: update config.guess
(cd gcc-3.4.6 && cp /usr/share/automake-1.11/config.guess .)
...
# DHH: set CONFIGURED_MAKEINFO = missing, since the makeinfo under
# Cygwin x86-64 doesn't seem to work.
sed 's,^CONFIGURED_MAKEINFO = makeinfo,CONFIGURED_MAKEINFO = missing,' < Makefile \
	> Fixed.Makefile
mv Makefile Save.Makefile
mv Fixed.Makefile Makefile
